...
export default function BestCPUPage({ onNavigate }) {
  ...
  useEffect(() => {
    filterCPUs();
    setCurrentPage(1);
  }, [searchQuery, cpus, filterBrand, filterMode]);
  ...
  const fetchCPUs = async () => {
    try {
      ...
      const withRank = data.map((cpu, idx) => {
        const rawRank = cpu.rank ? cpu.rank : `#${idx + 1}`;
        return { ...cpu, rank: rawRank };
      });
      ...
      const sorted = withRank
        ...
        .map((cpu, idx) => ({ ...cpu, rank: `#${idx + 1}` }));

      setCpus(sorted);
      setFilteredCpus(sorted);
    } catch (err) {
      console.error("Error fetching CPUs:", err);
      triggerNotif("Failed to load CPU data");
    } finally {
      setLoading(false);
    }
  };
  ...
  const filterCPUs = () => {
    ...
    if (query) {
      result = result.filter(cpu =>
        ...
        (String(cpu.rank || "") || "").toLowerCase().includes(query) ||
        ((cpu.detail || "").toLowerCase().includes(query))
      );
    }

    if (filterBrand && filterBrand !== "all") {
      result = result.filter(cpu => String(cpu.brand).toLowerCase() === filterBrand.toLowerCase());
    }
  ...
  const totalPages = Math.max(1, Math.ceil(filteredCpus.length / itemsPerPage));
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const currentCpus = filteredCpus.slice(startIndex, endIndex);
  ...
  const addOrUpdateCPU = async () => {
    ...
    try {
      if (editingId) {
        await cpuService.update(editingId, form);
        triggerNotif("The CPU has been successfully updated!");
      } else {
        const nextRank = cpus.length + 1;
        await cpuService.create({ ...form, rank: `#${nextRank}` });
        triggerNotif("The CPU has been successfully added!");
      }
      ...
      triggerNotif("Failed to save CPU data");
    }
  };
  ...
  const deleteCPU = async (id) => {
    if (!window.confirm("Are you sure you want to delete this CPU?")) return;

    try {
      await cpuService.delete(id);
      triggerNotif("The CPU has been successfully deleted!");
      await reorderRanks();
    } catch (err) {
      console.error("Error deleting CPU:", err);
      triggerNotif("Failed to delete CPU");
    }
  };
    ...
    try {
      ...
      for (let i = 0; i < sorted.length; i++) {
        ...
        if (sorted[i].rank !== newRank) {
          await cpuService.update(sorted[i].id, { rank: newRank });
        }
      }
      ...
      triggerNotif("Failed to sort CPU rank");
    }
  };

  const availableBrands = Array.from(new Set(cpus.map(c => (c.brand || "").trim()).filter(Boolean)));
    ...
    <div style={{ minHeight: "100vh", background: "#0f172a", color: "white" }}>
      ...
      <div className="header-container" style={{
        ...
        >
          ...
          {showForm ? "Close Form" : "Add New CPU"}
        </button>
      </div>
      ...
      <div className="search-container" style={{
        ...
        <div style={{ marginTop: "0.9rem", display: "flex", justifyContent: "center" }}>
          <p style={{
            ...
            Showing {filteredCpus.length} CPUs{filterBrand !== "all" ? ` • Brand: ${filterBrand}` : ""}{filterMode !== "all" ? ` • Mode: ${filterMode}` : ""}
          </p>
        </div>
      </div>
      ...
      {showForm && (
        <div className="form-container" style={{
          ...
          <textarea
            placeholder="CPU Details (cores, clock speed, generation, dll)"
            value={form.detail}
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "2rem 3rem 3rem" }}>
        {filteredCpus.length === 0 && !loading && (
          <div style={{ textAlign: "center", marginTop: "3rem" }}>
            <p style={{ fontSize: "1.4rem", opacity: 0.7 }}>
              {searchQuery
                ? `There is no CPU compatible with "${searchQuery}"`
                : "There is no CPU data. Click 'Add New CPU' to add."
              }
            </p>
        ...
        <div className="cpu-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: "2rem" }}>
          {currentCpus.map((cpu) => (
            <div key={cpu.id} style={{
              background: "#1f2937",
              borderRadius: "1.5rem",
              overflow: "hidden",
              ...
              <div style={{ padding: "1.5rem", display: "flex", alignItems: "center", gap: "1rem" }}>
                <img
                  src={cpu.img || "https://via.placeholder.com/80x80/0066ff/ffffff?text=CPU"}
                  alt={cpu.model}
                  style={{ width: "80px", height: "80px", borderRadius: "1rem", objectFit: "cover" }}
                  onError={(e) => { e.target.src = "https://via.placeholder.com/80x80/0066ff/ffffff?text=CPU"; }}
                />
                <div style={{ flex: 1 }}>
                  <h3 style={{ fontSize: "1.5rem", fontWeight: 700, margin: "0 0 0.5rem" }}>{cpu.model}</h3>
                  <p style={{ opacity: 0.8, margin: "0 0 0.5rem", fontSize: "1rem" }}>{cpu.brand}</p>
                  <p style={{ fontSize: "1.8rem", fontWeight: 900, color: "#60a5fa" }}>{cpu.rank}</p>
                </div>
              </div>

              <div style={{ padding: "0 1.5rem", flex: 1 }}>
                <p style={{ fontSize: "0.92rem", opacity: 0.75, lineHeight: "1.6", marginBottom: "1rem" }}>
                  {cpu.detail || "No detail available"}
                </p>
              </div>
              ...
              <div style={{
                ...
                <button
                  onClick={() => onNavigate('detail-cpu', cpu)}
                  style={{
                    background: "#10b981",
                    color: "white",
      ...
      {filteredCpus.length > 0 && (
        <div className="pagination-container" style={{
          ...
          <p className="pagination-info" style={{
            ...
            Showing {Math.min(startIndex + 1, filteredCpus.length)} - {Math.min(endIndex, filteredCpus.length)} of {filteredCpus.length} CPUs
          </p>
