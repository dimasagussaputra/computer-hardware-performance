...
export default function BestUSBPage({ onNavigate }) {
  const [usbs, setUsbs] = useState([]);
  const [filteredUsbs, setFilteredUsbs] = useState([]);
  ...
  useEffect(() => {
    filterUSBs();
    setCurrentPage(1);
  }, [searchQuery, usbs, filterBrand, filterMode]);
  ...
  const fetchUSBs = async () => {
    try {
      setLoading(true);
      const data = await usbService.getAll();
      const withRank = data.map((usb, idx) => {
        const rawRank = usb.rank ? usb.rank : `#${idx + 1}`;
        return { ...usb, rank: rawRank };
      });
      ...
      const sorted = withRank
        ...
        .map((usb, idx) => ({ ...usb, rank: `#${idx + 1}` }));

      setUsbs(sorted);
      setFilteredUsbs(sorted);
    } catch (err) {
      console.error("Error fetching USBs:", err);
      triggerNotif("Failed to load USB data");
    } finally {
      setLoading(false);
    }
  };
  ...
  const filterUSBs = () => {
    ...
    let result = [...usbs];
    if (query) {
      result = result.filter(usb =>
        (usb.brand || "").toLowerCase().includes(query) ||
        (usb.model || "").toLowerCase().includes(query) ||
        (String(usb.rank || "") || "").toLowerCase().includes(query) ||
        ((usb.detail || "").toLowerCase().includes(query))
      );
    }

    if (filterBrand && filterBrand !== "all") {
      result = result.filter(usb => String(usb.brand).toLowerCase() === filterBrand.toLowerCase());
    }
    ...
    setFilteredUsbs(result);
  };

  const totalPages = Math.max(1, Math.ceil(filteredUsbs.length / itemsPerPage));
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const currentUsbs = filteredUsbs.slice(startIndex, endIndex);
  ...
  const addOrUpdateUSB = async () => {
    ...
    try {
      if (editingId) {
        await usbService.update(editingId, form);
        triggerNotif("The USB has been successfully updated!");
      } else {
        const nextRank = usbs.length + 1;
        await usbService.create({ ...form, rank: `#${nextRank}` });
        triggerNotif("The USB has been successfully added!");
      }
      ...
      console.error("Error saving USB:", err);
      triggerNotif("Failed to save USB data");
    }
  };
  ...
  const deleteUSB = async (id) => {
    if (!window.confirm("Are you sure you want to delete this USB?")) return;

    try {
      await usbService.delete(id);
      triggerNotif("The USB has been successfully deleted!");
      await reorderRanks();
    } catch (err) {
      console.error("Error deleting USB:", err);
      triggerNotif("Failed to delete USB");
    }
  };
    ...
    try {
      ...
      for (let i = 0; i < sorted.length; i++) {
        ...
        if (sorted[i].rank !== newRank) {
          await usbService.update(sorted[i].id, { rank: newRank });
        }
      }
      ...
      triggerNotif("Failed to sort USB rank");
    }
  };

  const availableBrands = Array.from(new Set(usbs.map(u => (u.brand || "").trim()).filter(Boolean)));
    ...
    <div style={{ minHeight: "100vh", background: "#0f172a", color: "white" }}>
      ...
      <div className="header-container" style={{
        ...
        >
          ...
          {showForm ? "Close Form" : "Add New USB"}
        </button>
      </div>
      ...
      <div className="search-container" style={{
        ...
        <div style={{ marginTop: "0.9rem", display: "flex", justifyContent: "center" }}>
          <p style={{
            ...
            Showing {filteredUsbs.length} USBs{filterBrand !== "all" ? ` • Brand: ${filterBrand}` : ""}{filterMode !== "all" ? ` • Mode: ${filterMode}` : ""}
          </p>
        </div>
      </div>
      ...
      {showForm && (
        <div className="form-container" style={{
          ...
          <h3 style={{ marginBottom: "1.5rem", fontSize: "1.6rem", fontWeight: 700 }}>
            {editingId ? "Edit USB" : "Add New USB"}
          </h3>
          ...
          <textarea
            placeholder="USB Details (cores, clock speed, generation, dll)"
            value={form.detail}
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "2rem 3rem 3rem" }}>
        {filteredUsbs.length === 0 && !loading && (
          <div style={{ textAlign: "center", marginTop: "3rem" }}>
            <p style={{ fontSize: "1.4rem", opacity: 0.7 }}>
              {searchQuery
                ? `There is no USB compatible with "${searchQuery}"`
                : "There is no USB data. Click 'Add New USB' to add."
              }
            </p>
        ...
        <div className="usb-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: "2rem" }}>
          {currentUsbs.map((usb) => (
            <div key={usb.id} style={{
              background: "#1f2937",
              borderRadius: "1.5rem",
              overflow: "hidden",
              ...
              <div style={{ padding: "1.5rem", display: "flex", alignItems: "center", gap: "1rem" }}>
                <img
                  src={usb.img || "https://via.placeholder.com/80x80/0066ff/ffffff?text=USB"}
                  alt={usb.model}
                  style={{ width: "80px", height: "80px", borderRadius: "1rem", objectFit: "cover" }}
                  onError={(e) => { e.target.src = "https://via.placeholder.com/80x80/0066ff/ffffff?text=USB"; }}
                />
                <div style={{ flex: 1 }}>
                  <h3 style={{ fontSize: "1.5rem", fontWeight: 700, margin: "0 0 0.5rem" }}>{usb.model}</h3>
                  <p style={{ opacity: 0.8, margin: "0 0 0.5rem", fontSize: "1rem" }}>{usb.brand}</p>
                  <p style={{ fontSize: "1.8rem", fontWeight: 900, color: "#60a5fa" }}>{usb.rank}</p>
                </div>
              </div>

              <div style={{ padding: "0 1.5rem", flex: 1 }}>
                <p style={{ fontSize: "0.92rem", opacity: 0.75, lineHeight: "1.6", marginBottom: "1rem" }}>
                  {usb.detail || "No detail available"}
                </p>
              </div>
              ...
              <div style={{
                ...
                <button
                  onClick={() => onNavigate('detail-usb', usb)}
                  style={{
                    background: "#10b981",
                    color: "white",
      ...
      {filteredUsbs.length > 0 && (
        <div className="pagination-container" style={{
          ...
          <p className="pagination-info" style={{
            ...
            Showing {Math.min(startIndex + 1, filteredUsbs.length)} - {Math.min(endIndex, filteredUsbs.length)} of {filteredUsbs.length} USBs
          </p>
