...
export default function BestHDDPage({ onNavigate }) {
  ...
  useEffect(() => {
    filterHDDs();
    setCurrentPage(1);
  }, [searchQuery, hdds, filterBrand, filterMode]);
  ...
  const fetchHDDs = async () => {
    try {
      setLoading(true);
      const data = await hddService.getAll();
      const withRank = data.map((hdd, idx) => {
        const rawRank = hdd.rank ? hdd.rank : `#${idx + 1}`;
        return { ...hdd, rank: rawRank };
      });
      ...
      const sorted = withRank
        ...
        .map((hdd, idx) => ({ ...hdd, rank: `#${idx + 1}` }));

      setHdds(sorted);
      setFilteredHdds(sorted);
    } catch (err) {
      console.error("Error fetching HDDs:", err);
      triggerNotif("Failed to load HDD data");
    } finally {
      setLoading(false);
    }
  };
  ...
  const filterHDDs = () => {
    ...
    let result = [...hdds];
    if (query) {
      result = result.filter(hdd =>
        (hdd.brand || "").toLowerCase().includes(query) ||
        (hdd.model || "").toLowerCase().includes(query) ||
        (String(hdd.rank || "") || "").toLowerCase().includes(query) ||
        ((hdd.detail || "").toLowerCase().includes(query))
      );
    }

    if (filterBrand && filterBrand !== "all") {
      result = result.filter(hdd => String(hdd.brand).toLowerCase() === filterBrand.toLowerCase());
    }
    ...
    setFilteredHdds(result);
  };

  const totalPages = Math.max(1, Math.ceil(filteredHdds.length / itemsPerPage));
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const currentHdds = filteredHdds.slice(startIndex, endIndex);
  ...
  const addOrUpdateHDD = async () => {
    ...
    try {
      if (editingId) {
        await hddService.update(editingId, form);
        triggerNotif("The HDD has been successfully updated!");
      } else {
        const nextRank = hdds.length + 1;
        await hddService.create({ ...form, rank: `#${nextRank}` });
        triggerNotif("The HDD has been successfully added!");
      }
      ...
      console.error("Error saving HDD:", err);
      triggerNotif("Failed to save HDD data");
    }
  };
  ...
  const deleteHDD = async (id) => {
    if (!window.confirm("Are you sure you want to delete this HDD?")) return;

    try {
      await hddService.delete(id);
      triggerNotif("The HDD has been successfully deleted!");
      await reorderRanks();
    } catch (err) {
      console.error("Error deleting HDD:", err);
      triggerNotif("Failed to delete HDD");
    }
  };
    ...
    try {
      ...
      for (let i = 0; i < sorted.length; i++) {
        ...
        if (sorted[i].rank !== newRank) {
          await hddService.update(sorted[i].id, { rank: newRank });
        }
      }
      ...
      triggerNotif("Failed to sort HDD rank");
    }
  };

  const availableBrands = Array.from(new Set(hdds.map(h => (h.brand || "").trim()).filter(Boolean)));
    ...
    <div style={{ minHeight: "100vh", background: "#0f172a", color: "white" }}>
      ...
      <div className="header-container" style={{
        ...
        >
          ...
          {showForm ? "Close Form" : "Add New HDD"}
        </button>
      </div>
      ...
      <div className="search-container" style={{
        ...
        <div style={{ marginTop: "0.9rem", display: "flex", justifyContent: "center" }}>
          <p style={{
            ...
            Showing {filteredHdds.length} HDDs{filterBrand !== "all" ? ` • Brand: ${filterBrand}` : ""}{filterMode !== "all" ? ` • Mode: ${filterMode}` : ""}
          </p>
        </div>
      </div>
      ...
      {showForm && (
        <div className="form-container" style={{
          ...
          <h3 style={{ marginBottom: "1.5rem", fontSize: "1.6rem", fontWeight: 700 }}>
            {editingId ? "Edit HDD" : "Add New HDD"}
          </h3>
          ...
          <textarea
            placeholder="HDD Details (capacity, RPM, cache, dll)"
            value={form.detail}
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "2rem 3rem 3rem" }}>
        {filteredHdds.length === 0 && !loading && (
          <div style={{ textAlign: "center", marginTop: "3rem" }}>
            <p style={{ fontSize: "1.4rem", opacity: 0.7 }}>
              {searchQuery
                ? `There is no HDD compatible with "${searchQuery}"`
                : "There is no HDD data. Click 'Add New HDD' to add."
              }
            </p>
        ...
        <div className="hdd-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: "2rem" }}>
          {currentHdds.map((hdd) => (
            <div key={hdd.id} style={{
              background: "#1f2937",
              borderRadius: "1.5rem",
              overflow: "hidden",
              ...
              <div style={{ padding: "1.5rem", display: "flex", alignItems: "center", gap: "1rem" }}>
                <img
                  src={hdd.img || "https://via.placeholder.com/80x80/0066ff/ffffff?text=HDD"}
                  alt={hdd.model}
                  style={{ width: "80px", height: "80px", borderRadius: "1rem", objectFit: "cover" }}
                  onError={(e) => { e.target.src = "https://via.placeholder.com/80x80/0066ff/ffffff?text=HDD"; }}
                />
                <div style={{ flex: 1 }}>
                  <h3 style={{ fontSize: "1.5rem", fontWeight: 700, margin: "0 0 0.5rem" }}>{hdd.model}</h3>
                  <p style={{ opacity: 0.8, margin: "0 0 0.5rem", fontSize: "1rem" }}>{hdd.brand}</p>
                  <p style={{ fontSize: "1.8rem", fontWeight: 900, color: "#60a5fa" }}>{hdd.rank}</p>
                </div>
              </div>

              <div style={{ padding: "0 1.5rem", flex: 1 }}>
                <p style={{ fontSize: "0.92rem", opacity: 0.75, lineHeight: "1.6", marginBottom: "1rem" }}>
                  {hdd.detail || "No detail available"}
                </p>
              </div>
              ...
              <div style={{
                ...
                <button
                  onClick={() => onNavigate('detail-hdd', hdd)}
                  style={{
                    background: "#10b981",
                    color: "white",
      ...
      {filteredHdds.length > 0 && (
        <div className="pagination-container" style={{
          ...
          <p className="pagination-info" style={{
            ...
            Showing {Math.min(startIndex + 1, filteredHdds.length)} - {Math.min(endIndex, filteredHdds.length)} of {filteredHdds.length} HDDs
          </p>
