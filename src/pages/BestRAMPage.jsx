...
export default function BestRAMPage({ onNavigate }) {
  ...
  useEffect(() => {
    filterRAMs();
    setCurrentPage(1);
  }, [searchQuery, rams, filterBrand, filterMode]);
  ...
  const fetchRAMs = async () => {
    try {
      setLoading(true);
      const data = await ramService.getAll();

      const withRank = data.map((ram, idx) => {
        const rawRank = ram.rank ? ram.rank : `#${idx + 1}`;
        return { ...ram, rank: rawRank };
      });
      ...
      const sorted = withRank
        ...
        .map((ram, idx) => ({ ...ram, rank: `#${idx + 1}` }));

      setRams(sorted);
      setFilteredRams(sorted);
    } catch (err) {
      console.error("Error fetching RAMs:", err);
      triggerNotif("Failed to load RAM data");
    } finally {
      setLoading(false);
    }
  };
  ...
  const filterRAMs = () => {
    ...
    let result = [...rams];
    if (query) {
      result = result.filter(ram =>
        (ram.brand || "").toLowerCase().includes(query) ||
        (ram.model || "").toLowerCase().includes(query) ||
        (String(ram.rank || "") || "").toLowerCase().includes(query) ||
        ((ram.detail || "").toLowerCase().includes(query))
      );
    }

    if (filterBrand && filterBrand !== "all") {
      result = result.filter(ram => String(ram.brand).toLowerCase() === filterBrand.toLowerCase());
    }
    ...
    setFilteredRams(result);
  };

  const totalPages = Math.max(1, Math.ceil(filteredRams.length / itemsPerPage));
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const currentRams = filteredRams.slice(startIndex, endIndex);
  ...
  const addOrUpdateRAM = async () => {
    ...
    try {
      if (editingId) {
        await ramService.update(editingId, form);
        triggerNotif("The RAM has been successfully updated!");
      } else {
        const nextRank = rams.length + 1;
        await ramService.create({ ...form, rank: `#${nextRank}` });
        triggerNotif("The RAM has been successfully added!");
      }
      ...
      triggerNotif("Failed to save RAM data");
    }
  };
  ...
  const deleteRAM = async (id) => {
    if (!window.confirm("Are you sure you want to delete this RAM?")) return;

    try {
      await ramService.delete(id);
      triggerNotif("The RAM has been successfully deleted!");
      await reorderRanks();
    } catch (err) {
      console.error("Error deleting RAM:", err);
      triggerNotif("Failed to delete RAM");
    }
  };
    ...
    try {
      ...
      for (let i = 0; i < sorted.length; i++) {
        ...
        if (sorted[i].rank !== newRank) {
          await ramService.update(sorted[i].id, { rank: newRank });
        }
      }
      ...
      triggerNotif("Failed to sort RAM rank");
    }
  };

  const availableBrands = Array.from(new Set(rams.map(r => (r.brand || "").trim()).filter(Boolean)));
    ...
    <div style={{ minHeight: "100vh", background: "#0f172a", color: "white" }}>
      ...
      <div className="header-container" style={{
        ...
        >
          ...
          {showForm ? "Close Form" : "Add New RAM"}
        </button>
      </div>
      ...
      <div className="search-container" style={{
        ...
        <div style={{ marginTop: "0.9rem", display: "flex", justifyContent: "center" }}>
          <p style={{
            ...
            Showing {filteredRams.length} RAMs{filterBrand !== "all" ? ` • Brand: ${filterBrand}` : ""}{filterMode !== "all" ? ` • Mode: ${filterMode}` : ""}
          </p>
        </div>
      </div>
      ...
      {showForm && (
        <div className="form-container" style={{
          ...
          <textarea
            placeholder="RAM Details (type, speed, capacity, dll)"
            value={form.detail}
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "2rem 3rem 3rem" }}>
        {filteredRams.length === 0 && !loading && (
          <div style={{ textAlign: "center", marginTop: "3rem" }}>
            <p style={{ fontSize: "1.4rem", opacity: 0.7 }}>
              {searchQuery
                ? `There is no RAM compatible with "${searchQuery}"`
                : "There is no RAM data. Click 'Add New RAM' to add."
              }
            </p>
        ...
        <div className="ram-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: "2rem" }}>
          {currentRams.map((ram) => (
            <div key={ram.id} style={{
              ...
              <div style={{ padding: "1.5rem", display: "flex", alignItems: "center", gap: "1rem" }}>
                <img
                  src={ram.img || "https://via.placeholder.com/80x80/0066ff/ffffff?text=RAM"}
                  alt={ram.model}
                  style={{ width: "80px", height: "80px", borderRadius: "1rem", objectFit: "cover" }}
                  onError={(e) => { e.target.src = "https://via.placeholder.com/80x80/0066ff/ffffff?text=RAM"; }}
                />
                <div style={{ flex: 1 }}>
                  <h3 style={{ fontSize: "1.5rem", fontWeight: 700, margin: "0 0 0.5rem" }}>{ram.model}</h3>
                  <p style={{ opacity: 0.8, margin: "0 0 0.5rem", fontSize: "1rem" }}>{ram.brand}</p>
                  <p style={{ fontSize: "1.8rem", fontWeight: 900, color: "#60a5fa" }}>{ram.rank}</p>
                </div>
              </div>

              <div style={{ padding: "0 1.5rem", flex: 1 }}>
                <p style={{ fontSize: "0.92rem", opacity: 0.75, lineHeight: "1.6", marginBottom: "1rem" }}>
                  {ram.detail || "No detail available"}
                </p>
              </div>
              ...
              <div style={{
                ...
                <button
                  onClick={() => onNavigate('detail-ram', ram)}
                  style={{
                    background: "#10b981",
                    color: "white",
      ...
      {filteredRams.length > 0 && (
        <div className="pagination-container" style={{
          ...
          <p className="pagination-info" style={{
            ...
            Showing {Math.min(startIndex + 1, filteredRams.length)} - {Math.min(endIndex, filteredRams.length)} of {filteredRams.length} RAMs
          </p>
