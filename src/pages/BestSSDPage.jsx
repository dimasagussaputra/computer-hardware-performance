...
export default function BestSSDPage({ onNavigate }) {
  ...
  useEffect(() => {
    filterSSDs();
    setCurrentPage(1);
  }, [searchQuery, ssds, filterBrand, filterMode]);
  ...
  const fetchSSDs = async () => {
    try {
      setLoading(true);
      const data = await ssdService.getAll();
      const withRank = data.map((ssd, idx) => {
        const rawRank = ssd.rank ? ssd.rank : `#${idx + 1}`;
        return { ...ssd, rank: rawRank };
      });
      ...
      const sorted = withRank
        ...
        .map((ssd, idx) => ({ ...ssd, rank: `#${idx + 1}` }));

      setSsds(sorted);
      setFilteredSsds(sorted);
    } catch (err) {
      console.error("Error fetching SSDs:", err);
      triggerNotif("Failed to load SSD data");
    } finally {
      setLoading(false);
    }
  };
  ...
  const filterSSDs = () => {
    ...
    let result = [...ssds];
    if (query) {
      result = result.filter(ssd =>
        (ssd.brand || "").toLowerCase().includes(query) ||
        (ssd.model || "").toLowerCase().includes(query) ||
        (String(ssd.rank || "") || "").toLowerCase().includes(query) ||
        ((ssd.detail || "").toLowerCase().includes(query))
      );
    }

    if (filterBrand && filterBrand !== "all") {
      result = result.filter(ssd => String(ssd.brand).toLowerCase() === filterBrand.toLowerCase());
    }
    ...
    setFilteredSsds(result);
  };

  const totalPages = Math.max(1, Math.ceil(filteredSsds.length / itemsPerPage));
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const currentSsds = filteredSsds.slice(startIndex, endIndex);
  ...
  const addOrUpdateSSD = async () => {
    ...
    try {
      if (editingId) {
        await ssdService.update(editingId, form);
        triggerNotif("The SSD has been successfully updated!");
      } else {
        const nextRank = ssds.length + 1;
        await ssdService.create({ ...form, rank: `#${nextRank}` });
        triggerNotif("The SSD has been successfully added!");
      }
      ...
      console.error("Error saving SSD:", err);
      triggerNotif("Failed to save SSD data");
    }
  };
  ...
  const deleteSSD = async (id) => {
    if (!window.confirm("Are you sure you want to delete this SSD?")) return;

    try {
      await ssdService.delete(id);
      triggerNotif("The SSD has been successfully deleted!");
      await reorderRanks();
    } catch (err) {
      console.error("Error deleting SSD:", err);
      triggerNotif("Failed to delete SSD");
    }
  };
    ...
    try {
      ...
      for (let i = 0; i < sorted.length; i++) {
        ...
        if (sorted[i].rank !== newRank) {
          await ssdService.update(sorted[i].id, { rank: newRank });
        }
      }
      ...
      triggerNotif("Failed to sort SSD rank");
    }
  };

  const availableBrands = Array.from(new Set(ssds.map(s => (s.brand || "").trim()).filter(Boolean)));
    ...
    <div style={{ minHeight: "100vh", background: "#0f172a", color: "white" }}>
      ...
      <div className="header-container" style={{
        ...
        >
          ...
          {showForm ? "Close Form" : "Add New SSD"}
        </button>
      </div>
      ...
      <div className="search-container" style={{
        ...
        <div style={{ marginTop: "0.9rem", display: "flex", justifyContent: "center" }}>
          <p style={{
            ...
            Showing {filteredSsds.length} SSDs{filterBrand !== "all" ? ` • Brand: ${filterBrand}` : ""}{filterMode !== "all" ? ` • Mode: ${filterMode}` : ""}
          </p>
        </div>
      </div>
      ...
      {showForm && (
        <div className="form-container" style={{
          ...
          <h3 style={{ marginBottom: "1.5rem", fontSize: "1.6rem", fontWeight: 700 }}>
            {editingId ? "Edit SSD" : "Add New SSD"}
          </h3>
          ...
          <textarea
            placeholder="SSD Details (capacity, type, interface, dll)"
            value={form.detail}
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "2rem 3rem 3rem" }}>
        {filteredSsds.length === 0 && !loading && (
          <div style={{ textAlign: "center", marginTop: "3rem" }}>
            <p style={{ fontSize: "1.4rem", opacity: 0.7 }}>
              {searchQuery
                ? `There is no SSD compatible with "${searchQuery}"`
                : "There is no SSD data. Click 'Add New SSD' to add."
              }
            </p>
        ...
        <div className="ssd-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: "2rem" }}>
          {currentSsds.map((ssd) => (
            <div key={ssd.id} style={{
              background: "#1f2937",
              borderRadius: "1.5rem",
              overflow: "hidden",
              ...
              <div style={{ padding: "1.5rem", display: "flex", alignItems: "center", gap: "1rem" }}>
                <img
                  src={ssd.img || "https://via.placeholder.com/80x80/0066ff/ffffff?text=SSD"}
                  alt={ssd.model}
                  style={{ width: "80px", height: "80px", borderRadius: "1rem", objectFit: "cover" }}
                  onError={(e) => { e.target.src = "https://via.placeholder.com/80x80/0066ff/ffffff?text=SSD"; }}
                />
                <div style={{ flex: 1 }}>
                  <h3 style={{ fontSize: "1.5rem", fontWeight: 700, margin: "0 0 0.5rem" }}>{ssd.model}</h3>
                  <p style={{ opacity: 0.8, margin: "0 0 0.5rem", fontSize: "1rem" }}>{ssd.brand}</p>
                  <p style={{ fontSize: "1.8rem", fontWeight: 900, color: "#60a5fa" }}>{ssd.rank}</p>
                </div>
              </div>

              <div style={{ padding: "0 1.5rem", flex: 1 }}>
                <p style={{ fontSize: "0.92rem", opacity: 0.75, lineHeight: "1.6", marginBottom: "1rem" }}>
                  {ssd.detail || "No detail available"}
                </p>
              </div>
              ...
              <div style={{
                ...
                <button
                  onClick={() => onNavigate('detail-ssd', ssd)}
                  style={{
                    background: "#10b981",
                    color: "white",
      ...
      {filteredSsds.length > 0 && (
        <div className="pagination-container" style={{
          ...
          <p className="pagination-info" style={{
            ...
            Showing {Math.min(startIndex + 1, filteredSsds.length)} - {Math.min(endIndex, filteredSsds.length)} of {filteredSsds.length} SSDs
          </p>
